...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        private static void ApplySpawnPayload(NetworkIdentity identity, SpawnMessage msg)
        {
            ...
            {
                var payloadReader = NetworkReaderPool.GetReader(msg.payload);
        ...
        internal static void OnSpawn(SpawnMessage msg)
        {
            ...
            var identity = GetExistingObject(msg.netId);
        ...
        {
            if (GetPrefab(msg.assetId, out var prefab))
            {
                var obj = Object.Instantiate(prefab, msg.position, msg.rotation);
                if (LogFilter.Debug)
            ...
            if (spawnHandlers.TryGetValue(msg.assetId, out var handler))
            {
                var obj = handler(msg);
                if (obj == null)
                {
        ...
        {
            var spawnedId = SpawnSceneObject(msg.sceneId);
            if (spawnedId == null)
            {
        ...
        internal static void OnHostClientObjectHide(ObjectHideMessage msg)
        {
            ...
            if (NetworkIdentity.spawned.TryGetValue(msg.netId, out var localObject) && localObject != null)
